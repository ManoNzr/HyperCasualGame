...
public class ScalableObs : ObstacleScript
{
    ...
    protected override void MoveObstacle()
    {
        ...
        else if(dir.Contains("round")) pivot.transform.localScale = new Vector3(Mathf.PingPong(Time.time * Mathf.Clamp(speed, 0, 5), 5), Mathf.PingPong(Time.time * Mathf.Clamp(speed,0,5), 5), 3);
        return;
    }
}
